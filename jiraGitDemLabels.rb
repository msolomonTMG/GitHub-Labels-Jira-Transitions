		...
		while (i < jiraKeys.length) do
			...
			if currentLabel == "QAed" && jiraKey != nil
				if allLabels.find {|x| x['name'] == 'reviewed'} != nil
					...
					#QA'ed command: 
					system "curl -D- -u #{JIRA_USER_NAME}:#{JIRA_PASSWORD} -X POST --data '{\"update\": {\"comment\": [{\"add\": {\"body\": \"QA passed by #{actionUser}\"}}]}, \"transition\": {\"id\": \"91\"}}' -H \"Content-Type: application/json\" https://thrillistmediagroup.atlassian.net/rest/api/latest/issue/#{jiraKey}/transitions"
				...
				if allLabels.find {|x| x['name'] == 'QAed'} != nil
					...
					puts "Will try to move #{jiraKey} to Deploy Ready if it was named correctly.\n"
					#Reviewed command: 
					system "curl -D- -u #{JIRA_USER_NAME}:#{JIRA_PASSWORD} -X POST --data '{\"update\": {\"comment\": [{\"add\": {\"body\": \"Code review passed by #{actionUser}\"}}]}, \"transition\": {\"id\": \"101\"}}' -H \"Content-Type: application/json\" https://thrillistmediagroup.atlassian.net/rest/api/latest/issue/#{jiraKey}/transitions"
